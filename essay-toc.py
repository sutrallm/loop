...
def process_files_old(url, key, args):
    ...
    mappings = group_cn_by_dn(args.mapping)
    # print(group[args.section])

    for section, linkings in mappings.items():
        ...
        while retry < args.retry:
            ...
            question_path = os.path.join(args.output, f"{args.section}-question.txt")
            with open(question_path, 'w', encoding='utf-8') as out_file:
                out_file.write(question)
            ...
            output_path = os.path.join(args.output, f"{args.section}.txt")
            with open(output_path, 'w', encoding='utf-8') as out_file:
                out_file.write(content)
